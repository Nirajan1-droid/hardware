...
def update_plot(filename, fig, canvas, ax):
    """
    ...
    ax.cla()
    ax.plot(times, currents, label='Current (A)', color='blue')
    ax.plot(times, voltages, label='Voltage (V)', color='red')
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Values')
    ax.set_title(f'Real-time Data Plot - {filename}')
    ax.legend()
    ax.grid(True)
    canvas.draw()

class FileChangeHandler(FileSystemEventHandler):
    def __init__(self, filename, root, fig, canvas, ax):
    ...
    def on_modified(self, event):
        if event.src_path.endswith(self.filename):
            ...
            self.root.after(1, update_plot, self.filename, self.fig, self.canvas, self.ax)

def monitor_file(filename, root, fig, canvas, ax):
    """
    Monitors the specified file for changes and updates the Matplotlib plot.
    """
    event_handler = FileChangeHandler(filename, root, fig, canvas, ax)
    observer = Observer()
    ...
    read_data(filename)
    update_plot(filename, fig, canvas, ax)

    root.after(1000, check_for_updates, filename, fig,root, canvas, ax)

def check_for_updates(filename, fig, root,canvas, ax):
    """
    Checks for updates in the specified file and updates the plot.
    """
...
def create_plot_window(filename):
    ...
    monitor_file(filename, window, fig, canvas, ax)
...
def create_grid_view_window():
    ...
    for ax, (filename, data) in zip(axs, file_data.items()):
        ...
        canvas = FigureCanvasTkAgg(fig, master=grid_window)  # Instantiate canvas here
        canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)  # Pack canvas here

        monitor_file(filename, grid_window, fig, canvas, ax)

    fig.tight_layout()
    plt.show()  # Call plt.show() here if needed
